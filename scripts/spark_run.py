...
def setup_logger(log_file):
    logger = logging.getLogger("online-retail-baseline")
...
def parse_args():
    ...
    parser.add_argument("--app-name", default="online-retail-baseline")
    parser.add_argument("--run-label", default="1DataNode")
...
def main():
    ...
    try:
        ...
        raw_df = spark.read.option("header", True).option("inferSchema", True).csv(args.input)
        read_definition_sec = round(time.perf_counter() - read_started_at, 3)
        logger.info("DataFrame created with inferSchema=True in %.3fs", read_definition_sec)

        df = normalize_columns(raw_df)
        total_rows, total_rows_metrics = run_action(spark, logger, "count input rows", "rows-count", lambda: df.count())
        ...
        country_metrics, country_metrics_log = run_action(
            ...
            lambda: [
                row.asDict()
                for row in filtered_df.groupBy("country")
                .agg(
        ...
        monthly_metrics, monthly_metrics_log = run_action(
            ...
            lambda: [
                row.asDict()
                for row in filtered_df.withColumn("invoice_month", F.date_format("invoice_ts", "yyyy-MM"))
        ...
        metrics = {
            ...
            "actions": [total_rows_metrics, country_metrics_log, monthly_metrics_log],
            "samples": {
        ...
        logger.info("Spark app finished successfully")
        return 0
    finally:
        spark.stop()
